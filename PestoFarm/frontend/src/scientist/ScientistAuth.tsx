...
import { scientistAuthService } from '../services/authService';
...
const ScientistAuth = () => {
    ...
    const role = 'scientist'; // Role is fixed for this file
    ...
    const handleSendOtp = async (e: React.MouseEvent<HTMLButtonElement>) => {
        ...
        try {
            ...
            await scientistAuthService.sendOtp({ email, role: 'ROLE_SCIENTIST' });
        ...
        const performAuth = async () => {
            try {
                if (view === 'login') {
                    ...
                    const response = await scientistAuthService.login({
                        email,
                    ...
                    // Update auth context with response data (include jwt)
                    login({
                        email,
                        role: 'scientist',
                        password: password || '',
                    ...
                    // Redirect scientist to profile to complete details
                    navigate('/scientist-profile');
                } else {
                    // For createAccount, use signup then verify
                    const signupResponse = await scientistAuthService.signup({
                        email,
                        fullName,
                        password,
                        otp
                    ...
                    // Redirect to profile for completing scientist details
                    navigate('/scientist-profile');
                }
            } catch (error: any) {
                ...
                <div className="w-full max-w-md bg-white rounded-xl shadow-2xl p-8 sm:p-10 border border-gray-200">
                    ...
                    <div className="flex flex-col items-center mb-8">
                        ...
                        <h2 className="text-3xl font-bold text-gray-900 text-center">
                            {view === 'login' ? 'Scientist Sign In' : 'Scientist Account Registration'}
                        </h2>
                        <p className="text-gray-500 mt-1">
                            Access the PestoFarm Scientist Dashboard.
                        </p>
                    </div>
                        ...
                        >
                            {isLoading ? (
                                ...
                                <>
                                    ...
                                    {view === 'login' ? 'Login to Scientist Portal' : 'Create Scientist Account'}
                                </>
                            )}
                        </button>
                    </form>
